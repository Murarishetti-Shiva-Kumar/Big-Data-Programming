...
# Query2
# Query to count the tweets with Pfizer
# def get_query2():
#     plt.clf()
...
# Description of the tweet where it has vaccine as word in it
query_5 = spark.sql("select description from user where description like '%vaccine%' ")
query_5.show()
...
def get_query13():
    ...
    qpfizer = spark.sql(
        "select 'pfizer' as keyword, count(*) as Count from tweets where text like '%pfizer%'")
    qmoderna = spark.sql(
        "select 'modrena' as keyword, count(*) as Count from tweets where text like '%modrena%'")
    qjj = spark.sql(
        "select 'jj' as keyword , count(*) as Count from tweets where text like '%jj%'")
    query_13 = qpfizer.union(qmoderna).union(qjj)
    pd = query_13.toPandas()
    pd.to_csv('Query13.csv', index=False)
    pd.set_index("keyword", drop=True, inplace=True)
    pd.plot.bar()
    plt.xticks(rotation=0, ha="right")
    plt.title("Inspect Based on brand")
